  ...
  position: absolute;
        ...
        <div class="wrap">
            ...
            <div class=make_plan>
                <form name="formSend" action="test1.php" method="post" style="text-align:center;">
                    <span><input class ="date" type="date" name="from" min="2020-01-01" max="2021-12-31"value=<?=$re_3?>></span>
                    <span><input class ="date" type="date" name="to" min="2020-01-01" max="2021-12-31"value=<?=$re_4?>></span>
                ...
                <span style="height: 62px;
                    ...
                    font-weight: bold;
                    margin-right: 15px;">
                    <input style="background: rgba(255,255,255,0);" type="submit" value="플랜 만들기" onclick="goForm()">
                </span>      
                </form>
                </div>
        </div>
    ...
    margin-left: 15px;
            ...
            <ul class="container" style="background-color: rgba( 0, 0, 0, 0.5 ); ">
                ...
                <li><div class="done"><img src="./css/icon/done.png" style="width: 130px; margin-left: 10px; margin-top: 10px;"></div></li>
                ...
                <input type="button" class="gomypage" onclick="goForm2()">
            </form>
            <ul>
        </div>
    ...
    // ===== 마커 부분 ====
        ...
        <?php
            ...
            $sql2 ="select * from oil where title like '%{$temp1}%'";
            ...
            gas_stationPositions.push({
                title:"<?=$gas_title?>",
                latlng:new kakao.maps.LatLng(<?= $gas_lat; ?>, <?= $gas_longt; ?>)});
            <?php
            }
        ?>
        // 맛집
        ...
        <?php
            ...
            restaurantPositions.push({
                title:"<?=$res_title?>",
                latlng:new kakao.maps.LatLng(<?= $res_lat; ?>, <?= $res_longt; ?>)});
            <?php
            }
        ?>
        ...
        function createGas_stationMarkers() {
            for (var i = 0; i < gas_stationPositions.length; i++) {
                ...
                // 마커이미지와 마커를 생성합니다
                var markerImage = createMarkerImage(markerImageSrc, imageSize, imageOptions),
                    marker = createMarker(gas_stationPositions[i].latlng, markerImage);
                     // 마커에 표시할 인포윈도우를 생성합니다 
                var infowindow = new kakao.maps.InfoWindow({
                        content: gas_stationPositions[i].title // 인포윈도우에 표시할 내용
                    });
            ...
            for (var i = 0; i < restaurantPositions.length; i++) {
                ...
                var markerImage = createMarkerImage(markerImageSrc, imageSize, imageOptions),
                    marker = createMarker(restaurantPositions[i].latlng, markerImage);
                      // 마커에 표시할 인포윈도우를 생성합니다 
                var infowindow = new kakao.maps.InfoWindow({
                        content: restaurantPositions[i].title // 인포윈도우에 표시할 내용
                    });
