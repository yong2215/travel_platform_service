...
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR&display=swap" rel="stylesheet">
    ...
    <div id="header">
            <div class="wrap">
                ...
                <ul class="cate_list" >
                <form name="formSend" action="backup.php" method="post">
...
var overlayOn = false;  // 지도위에 로드뷰 오버레이가 추가된 상태 가지고 있을 변수
        ...
        //     $temp1 = "강릉시";
        //     $re_2 = 0;
        //     if ($re_2 != 0) { // test1으로 도는 경우
        //         $conn = mysqli_connect("dddlee1234.iptime.org","wodud2970","jy7950","wodud2970",8816);
        //         $sql ="select * from startMap where area like '%{$temp1}%'";
        //         $result = mysqli_query($conn,$sql); 
        //         while($row=mysqli_fetch_array($result)){
        //             $a1=(double)$row['lat'];
        //             $b1=(double)$row['longt'];
        ...
        var mapCenter = new kakao.maps.LatLng(36.132087, 128.097464)
		var options = {
			center: mapCenter,
			level: 12
        };
        ...
        <?php
            $conn = mysqli_connect("dddlee1234.iptime.org","wodud2970","jy7950","wodud2970",8816);
            $sql ="select * from travel where addr like '%울산%' and tagname not like '%음식%'";
            $result = mysqli_query($conn,$sql);
            $n=1;
            while($row=mysqli_fetch_array($result)){
    ...
    // ===== 근무지 다각형 부분 (폴리곤 : 원주) =====
        ...
        while($row=mysqli_fetch_array($result)){
            ...
            $b=round((double)$row['longt'],6);
        ?>
            polygonPath.push(new kakao.maps.LatLng(<?= $a; ?>, <?= $b; ?>));        
        <?php
            $n++;
        }
        ?>
    ...
    // ===== 여행지 다각형 부분 (폴리곤 : 경상남도 ) =====
        ...
        <?php
        $conn = mysqli_connect("dddlee1234.iptime.org","wodud2970","jy7950","wodud2970",8816);
        $sql ="select * from areaPolygon where area like '%울산%'";
        $result = mysqli_query($conn,$sql);
        $n=1;
        while($row=mysqli_fetch_array($result)){
            $a=round((double)$row['lat'],6);
            $b=round((double)$row['longt'],6);
        ?>
            polygonPath1.push(new kakao.maps.LatLng(<?= $a; ?>, <?= $b; ?>));        
        <?php
            $n++;
        }
        ?>
