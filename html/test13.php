...
<link href="https://fonts.googleapis.com/css2?family=Noto+Sans+KR&display=swap" rel="stylesheet">
    ...
    <div id="header">
        <div class="wrap">
            ...
            <ul class="cate_list" >
            <form action="test1.php" method="post" name="formSendSend">
                        ...
                        <span style="height: 62px;
                                ...
                                <input style="background: rgba(255,255,255,0);" type="button" value="플랜 만들기" class="returnPage" onclick="sendsend()">
                        </span>   
                        </form>
						<span style="height: 62px;
							line-height: 62px;
    ...
    //     var areas = []; 
    //     <?php
    //         $world = explode( ' ', $temp1)[1];
    //         $conn = mysqli_connect("dddlee1234.iptime.org","wodud2970","jy7950","wodud2970",8816);
    //         $sql ="select * from areaPolygon where area like '%{$world}%'";
    //         $result = mysqli_query($conn,$sql);
    //         $n=1;
    //         while($row=mysqli_fetch_array($result)){
    //             $a=round((double)$row['lat'],6);
    ...
    // ===== 다음 페이지 보내는 부분 =====
        function send() {
            formSend = document.formSend;
            formSend.array.value = applyList;
            formSend.submit();
        }
    // ===== 갱신시 보내는 부분 =====
        function sendsend() {
            form2 = document.formSendSend;
